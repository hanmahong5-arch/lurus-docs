...
- API Host の末尾に**スラッシュが付いていない**ことを確認してください（`https://hub.lurus.cn` であり、`https://hub.lurus.cn/` ではありません）
- API Key の形式が正しいことを確認してください（`sk-` で始まる）
- [Lurus コンソール](https://hub.lurus.cn) で Key のステータスが「有効」になっていることを確認してください
