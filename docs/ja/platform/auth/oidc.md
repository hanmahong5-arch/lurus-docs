...
すべてのエンドポイントは `https://auth.lurus.cn` を Base URL とします。
...
`verification_uri`（`https://auth.lurus.cn/device`）+ `user_code` を表示するか、`verification_uri_complete` を QR コードで読み取らせます。タイムアウトは 5 分。
