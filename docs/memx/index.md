...
MemX 的 <Term t="Reflector">Reflector</Term> 引擎采用 hybrid 混合模式（规则预筛选 + LLM 精炼），从对话中自动识别 5 种知识模式：
...
<Term t="Curator">Curator</Term> 引擎通过<Term t="Cosine Similarity">余弦相似度</Term>自动处理重复知识：
...
- <Term t="PII">API Key</Term>（OpenAI / Anthropic / AWS / GitHub）
- 数据库连接串
- PEM 私钥 / JWT Token
- 自定义正则规则（可扩展）
